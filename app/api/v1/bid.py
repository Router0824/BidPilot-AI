...
from app.application.review_export_service import ReviewService, ExportService, FixerService
...
async def coverage_matrix(
    ...
    return APIResponse(data=await EvidenceGraphService(db).list_matrix(project_id))


@router.post("/coverage-matrix/rebuild")
async def rebuild_coverage_matrix(
    project_id: str,
    db: AsyncSession = Depends(get_db), user: dict = Depends(require_auth),
):
    result = await EvidenceGraphService(db).rebuild_project_links(project_id)
    await EnterpriseService(db).audit("evidence_graph", project_id, "rebuild", user, None, result)
    return APIResponse(data=result)


@router.get("/requirements/{requirement_id}/evidence-chain")
async def requirement_evidence_chain(
    project_id: str,
    requirement_id: str,
    db: AsyncSession = Depends(get_db), user: dict = Depends(require_auth),
):
    result = await EvidenceGraphService(db).evidence_chain(project_id, requirement_id)
    if not result:
        raise HTTPException(404, "证据链不存在")
    return APIResponse(data=result)
...
async def run_review(
    ...
    return APIResponse(data={
        ...
        "findings": [{
            "id": f.id, "issue_id": f.id, "issue_type": f.finding_type,
...
@router.get("/fixes")
async def list_fix_attempts(
    project_id: str,
    db: AsyncSession = Depends(get_db), user: dict = Depends(require_auth),
):
    attempts = await FixerService(db).list_attempts(project_id)
    return APIResponse(data=[{
        "id": a.id,
        "issue_id": a.issue_id,
        "section_id": a.section_id,
        "draft_version_id": a.draft_version_id,
        "issue_type": a.issue_type,
        "risk_level": a.risk_level,
        "auto_fix_allowed": a.auto_fix_allowed,
        "status": a.status,
        "reason": a.reason,
        "before_content": a.before_content,
        "after_content": a.after_content,
        "diff": a.diff,
...
@router.post("/reviews/findings/{finding_id}/fix")
async def fix_finding(
    ...
    try:
        attempt = await FixerService(db).fix_issue(project_id, finding_id, user, apply)
    except ValueError as e:
        raise HTTPException(404, str(e))
    await EnterpriseService(db).audit("fix_attempt", project_id, "fix_issue", user, None, {
        "issue_id": finding_id,
        "attempt_id": attempt.id,
        "status": attempt.status,
        "auto_fix_allowed": attempt.auto_fix_allowed,
    })
    return APIResponse(data={
        "id": attempt.id,
        ...
        "auto_fix_allowed": attempt.auto_fix_allowed,
        "requires_human_confirmation": attempt.requires_human_confirmation,
        "before_content": attempt.before_content,
        "after_content": attempt.after_content,
        "diff": attempt.diff,
