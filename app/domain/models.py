...
class RequirementEvidenceLink(Base):
    ...
    requirement_type = Column(String(30))
    score_weight = Column(Float)
    mandatory = Column(Boolean, default=False)
    risk_level = Column(String(20), default=RiskLevel.MEDIUM.value)
    confidence = Column(Float)
    target_section_id = Column(String)
    target_section_title = Column(String(255))
    knowledge_evidence_ids = Column(JSON)
    generated_content_id = Column(String)
    review_issue_ids = Column(JSON)
    coverage_status = Column(String(30), default=CoverageStatus.MISSING.value)
...
class WorkflowPlan(Base):
    ...
    goal = Column(Text)
    selected_nodes = Column(JSON)
    skipped_nodes = Column(JSON)
    ...
    created_by = Column(String(100), default="planner_agent")
    created_at = Column(DateTime, default=utcnow)


class WorkflowImpactPreview(Base):
    __tablename__ = "workflow_impact_previews"

    id = Column(String, primary_key=True, default=new_id)
    project_id = Column(String, ForeignKey("projects.id"), nullable=False)
    change_type = Column(String(50), nullable=False)
    changed_resources = Column(JSON)
    changed_signals = Column(JSON)
    affected_nodes = Column(JSON)
    unaffected_nodes = Column(JSON)
    dependency_edges = Column(JSON)
...
class FixAttempt(Base):
    ...
    id = Column(String, primary_key=True, default=new_id)
    project_id = Column(String, ForeignKey("projects.id"), nullable=False)
    issue_id = Column(String, ForeignKey("review_findings.id"), nullable=False)
    section_id = Column(String)
    draft_version_id = Column(String)
    issue_type = Column(String(50))
    risk_level = Column(String(20))
    auto_fix_allowed = Column(Boolean, default=False)
    ...
    created_by = Column(String(100), default="fixer_agent")
    created_at = Column(DateTime, default=utcnow)
