<template>
  <div class="dashboard">
    ...
    <div v-else class="project-grid">
      ...
      <div v-if="projects.length === 0" class="empty">暂无项目，点击"新建项目"开始</div>
    </div>
...
.hero-strip {
  ...
  border-radius: 8px;
  background:
    linear-gradient(100deg, rgba(255,255,255,.94), rgba(255,255,255,.78)),
    repeating-linear-gradient(90deg, rgba(14,43,79,.08) 0, rgba(14,43,79,.08) 1px, transparent 1px, transparent 28px);
...
.hero-strip::after {
  ...
  width: 190px;
  height: 70px;
  border-top: 2px solid rgba(45,156,189,.26);
...
.project-card {
  position: relative;
  background: white; border-radius: 8px; padding: 18px; cursor: pointer;
  box-shadow: 0 2px 8px rgba(0,0,0,0.06); transition: box-shadow 0.2s, transform .16s;
  overflow: hidden;
}
.project-card::before {
  content: "";
...
.status {
  font-size: 11px; padding: 4px 9px; border-radius: 10px; background: #e7f2f5; color: #0f3460; white-space: nowrap;
}
.metric-line {
  display: grid;
