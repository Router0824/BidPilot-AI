          ...
          <div v-for="msg in messages" :key="msg.id" :class="['message', msg.role]">
            <div class="bubble">
              ...
              <div v-if="msg.meta?.confidence" class="confidence-line">
                可信度 {{ Math.round(msg.meta.confidence * 100) }}% · 引用 {{ dedupedCitations(msg.citations).length }} 条
              </div>
              <div v-if="dedupedCitations(msg.citations).length" class="citations">
                <div v-for="(citation, idx) in dedupedCitations(msg.citations)" :key="idx" class="citation">
...
function dedupedCitations(citations = []) {
  const seen = new Set()
  const seenSnippets = new Set()
  return citations.filter((citation) => {
    const snippet = String(citation.snippet || '').replace(/\s+/g, ' ').trim()
    const snippetKey = snippet.slice(0, 220)
    const key = [
      citation.type || '',
      citation.source || '',
      citation.page || '',
    ...
    if (seen.has(key) || (snippetKey && seenSnippets.has(snippetKey))) return false
    seen.add(key)
